...
squads <- dbGetQuery(con, paste0("SELECT * FROM SquadDisplayView ORDER BY Region, [Placement Rank]")) 
...
region_table <- function(region) {
  ...
  datatable(region, 
    ...
    formatStyle('Placement Rank', background = styleInterval(breaks, colors)) %>%
    formatStyle('Power Rank', background = styleInterval(breaks, colors)) %>%
    formatStyle('Chemistry Score', fontWeight = styleInterval( c(-5, 5), c('bold', 'normal', 'bold')))
}
